...
def plot_signals(noisy_signals, cleaned_signals, SG_denoised, skin_gen_from_cleaned, skin_gen_from_noisy, skin_gen_from_SG, gt_signals, SNR_list, num_samples=5, save_path="./results/"):
    ...
    # =========== figure for all ===============
    fig, axs = plt.subplots(num_samples, 4, figsize=(30, num_samples * 4))
    
    for i, idx in enumerate(selected_indices):
        axs[i, 0].plot(np.linspace(800, 1790, 1981), noisy_signals[idx], label="Raw spectra", color='black', linewidth=2)
        axs[i, 0].set_title(f"Raw spectra, SNR = {SNR_list[idx]:.2f}", fontsize=22)
        axs[i, 0].tick_params(axis='both', which='major', labelsize=20)
        
        axs[i, 1].plot(np.linspace(800, 1790, 1981), noisy_signals[idx], label="Raw", color='gray', linewidth=2, alpha=0.5)
        axs[i, 1].plot(np.linspace(800, 1790, 1981), SG_denoised[idx], label="SG", color='#d62728', linewidth=2)
        axs[i, 1].plot(np.linspace(800, 1790, 1981), cleaned_signals[idx], label="NL", color='#1f77b4', linewidth=2.5)
        axs[i, 1].set_title(f"Denoised spectra", fontsize=22)
        axs[i, 1].tick_params(axis='both', which='major', labelsize=20)
        axs[i, 1].legend(fontsize=20)
        
        axs[i, 2].plot(np.linspace(800, 1790, 1981), skin_gen_from_noisy[idx], label="Raw-SkinGen", color='#2ca02c', linewidth=2, alpha=1)
        axs[i, 2].plot(np.linspace(800, 1790, 1981), skin_gen_from_SG[idx], label="SG-SkinGen", color='#d62728', linewidth=2)
        axs[i, 2].plot(np.linspace(800, 1790, 1981), skin_gen_from_cleaned[idx], label="NL-SkinGen", color='#1f77b4', linewidth=2.5)
        axs[i, 2].set_title(f"Skin generation output", fontsize=22)
        axs[i, 2].tick_params(axis='both', which='major', labelsize=20)
        ...
        axs[i, 3].set_title(f"Pure spectra", fontsize=22)
        axs[i, 3].tick_params(axis='both', which='major', labelsize=20)

    plt.tight_layout()
    plt.show()
    plt.savefig(os.path.join(save_path, "signal_skin_gen_from_all_all_test.jpg"))

    fig, axs = plt.subplots(num_samples, 1, figsize=(7, num_samples * 4))
    for i, idx in enumerate(selected_indices):
        axs[i].plot(np.linspace(800, 1790, 1981), gt_signals[idx] - skin_gen_from_cleaned[idx], label="Residual_from_NL", color='#1f77b4')
        axs[i].plot(np.linspace(800, 1790, 1981), gt_signals[idx] - skin_gen_from_noisy[idx], label="Residual_from_Raw", color='#2ca02c')
        axs[i].plot(np.linspace(800, 1790, 1981), gt_signals[idx] - skin_gen_from_SG[idx], label="Residual_from_SG", color='#d62728')
