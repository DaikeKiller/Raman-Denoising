...
def plot(signal1, singal2, original, gt, SNR_list):
    ...
    plt.plot(wvn, signal1[idx] / np.max(original[idx]) + 2*offset, label='Traditional Model', linestyle='-')
    plt.plot(wvn, singal2[idx] / np.max(original[idx]) + offset, label='Deep Learning Model', linestyle='-')
    plt.plot(wvn, gt[idx] / np.max(original[idx]), label='Ground Truth', linestyle='-')
